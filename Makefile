...
## MCP server registration happens automatically via GKE auto-discovery once deployed
## with the registry.gke.io/functional-type + a2a-protocol.org/agent-card labels/annotations
## — nothing to publish manually here.
sync-artifacts-servers:
	@echo "==> MCP server registration with the Agent Registry is automatic via GKE auto-discovery."
	@echo "    No manual publish step needed — see helm/jarvis/templates/mcp-deployment.yaml."

## Publish all skills under artifacts/skills/ to the GCP Agent Registry Skills API.
## Requires `gcloud auth login` (or ADC) with agentregistry.read-write on the target project.
## See scripts/publish-skill.sh for the per-skill create/publish/activate logic.
