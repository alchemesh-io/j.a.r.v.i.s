...
- [x] 1.2 Extend `WorkerState` enum in `backend/app/models/enums.py` with `stopped` and `error`
...
- [x] 2.5 Add `POST /api/v1/workers/{id}/stop` route: validate state ∈ {`working`, `waiting_for_human`} (or already `stopped`), reject for ephemeral with HTTP 409, delete pod + service via `k8s.delete_worker_pod_only`, set DB state `stopped`
- [x] 2.6 Add `POST /api/v1/workers/{id}/restart` route: validate state ∈ {`stopped`, `error`} and `mode == stateful`, rebuild pod spec from worker row, call `k8s.create_worker_pod` + `create_worker_service`, set DB state to `initialized`
...
- [x] 3.3 Add `delete_worker_pod_only(worker_id)` and `delete_worker_service(worker_id)` for stop (Pod + Service only, never PVC)
...
- [x] 6.2 Add `'stopped'` and `'error'` to the `WorkerState` union type in the same file
- [x] 6.3 Add `stopWorker(id: string)` and `restartWorker(id: string)` functions hitting the new endpoints
