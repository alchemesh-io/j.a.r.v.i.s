...
**Correction post-deployment:** the design originally also called for `automount_service_account_token=False` on the worker pod spec, on the theory that a mounted-but-unused token is one less thing to reason about. Deployed against the real `t2-d-sbx-arch` cluster, this combination (`serviceAccountName` set + automount disabled) is hard-rejected by that cluster's own `deny-automount-token-without-sa` ValidatingAdmissionPolicy — a baseline hardening policy, not something introduced by this change — which treats it as a likely misconfiguration and denies pod creation outright (a real production incident: every worker creation 503'd until this was reverted). The token is now left mounted; safety still holds entirely through zero RoleBindings on `jarvis-worker` — confirmed via `kubectl auth can-i --list --as=system:serviceaccount:jarvis:jarvis-worker` against a live worker pod, which returns only the cluster's baseline discovery/health endpoints available to any authenticated identity, nothing more.

This means `kubectl`/`k9s`/`kubectx` inside a worker pod, out of the box, get `Forbidden` against the in-cluster API (a token with zero bindings) — same posture as if they weren't installed at all from a permissions standpoint. They remain fully useful for everything that isn't the in-cluster API: `kubectl` against a different context a task explicitly configures, `k9s`/`kubectx` for local kubeconfig inspection, `helm template`/`helm lint` (no cluster needed), etc.
