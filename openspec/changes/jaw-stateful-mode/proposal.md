...
We need a way to **stop** a worker (planned stop or post-failure freeze) without losing its working state, and to **resume** it later attached to the same data — so that long Claude Code sessions can survive transient pod failures, node drains, and explicit human pauses.
...
- **New `stopped` worker state** added to `WorkerState`, distinct from `archived`. `stopped` means: the pod and service are deleted, but the worker DB row, PVC, and all data are kept; the worker can be resumed.
...
- **Stop / Restart API**: new endpoints `POST /api/v1/workers/{id}/stop` and `POST /api/v1/workers/{id}/restart`. Pause deletes the pod + service (keeps PVC + DB row + state=`stopped`). Resume re-creates the pod + service attached to the existing PVC.
...
- `skill-worker-integration`: skill pulls SHALL be skipped on a stateful resume when the skill directory already exists under the persisted `~/.claude/skills/<name>/` (no re-pull on restart).
...
- **Frontend**:
  - `pages/Workers/`: new mode badge on worker cards; stop / resume buttons gated on state + mode.
