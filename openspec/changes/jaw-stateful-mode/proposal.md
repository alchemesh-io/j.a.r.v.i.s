...
We need a way to **stop** a worker (planned stop or post-failure freeze) without losing its working state, and to **restart** it later attached to the same data — so that long Claude Code sessions can survive transient pod failures, node drains, and explicit human stops.
...
- **New `stopped` worker state** added to `WorkerState`, distinct from `archived`. `stopped` means: the pod and service are deleted, but the worker DB row, PVC, and all data are kept; the worker can be restarted.
...
- **Stop / Restart API**: new endpoints `POST /api/v1/workers/{id}/stop` and `POST /api/v1/workers/{id}/restart`. Stop deletes the pod + service (keeps PVC + DB row + state=`stopped`). Restart re-creates the pod + service attached to the existing PVC.
- **Re-run on error**: `POST /api/v1/workers/{id}/restart` is also valid when a stateful worker is in `error` state — the entrypoint detects the existing PVC contents and skips re-cloning / re-pulling skills.
- **Idempotent worker entrypoint**: `entrypoint.sh` SHALL detect already-cloned repositories (`<repo>/.git` present), already-pulled skills (`~/.claude/skills/<name>/` present), and an existing `~/.claude/projects/` directory and SHALL skip those initialization steps so restart is fast and non-destructive (no re-clone, no overwriting of session JSONL files). Claude config files coming from the ConfigMap (`policy-limits.json`, `remote-settings.json`, `settings.json`, `~/.claude.json`) SHALL be re-applied on every start so updates from the host take effect, while `projects/` and `skills/` SHALL never be overwritten.
- **Worker pod failure detection**: the backend's pod-status path SHALL map Kubernetes pod phase `Failed` (or container `terminated` with non-zero `exitCode`) to DB state `error`, so the UI can offer "Restart" instead of "Delete".
- **UI controls** (Workers page card and per-task worker action menu): a Stop button on running workers, a Restart button on `stopped` and `error` stateful workers, plus a mode badge ("Stateful" / "Ephemeral") on each worker card. A mode toggle is exposed in the Create Worker overlay (default: `ephemeral` for backwards compatibility).
- **Storage cleanup on archive/delete**: deleting a stateful worker (or transitioning it to `archived`) SHALL also delete its PVC; stop SHALL NOT.
- **BREAKING (worker schema)**: `WorkerResponse` gains a required `mode` field; existing rows are migrated with `mode='ephemeral'`.

## Capabilities

### New Capabilities
- `worker-stateful-mode`: stateful vs ephemeral worker mode, single PVC mounted at `/home/node`, stop / restart / re-run lifecycle endpoints, idempotent entrypoint, stopped/error states, and storage cleanup on archive.

### Modified Capabilities
- `skill-worker-integration`: skill pulls SHALL be skipped on a stateful restart when the skill directory already exists under the persisted `~/.claude/skills/<name>/` (no re-pull on restart).

## Impact

- **Backend**:
  - `app/models/enums.py`: new `WorkerMode` enum (`ephemeral`, `stateful`); two new `WorkerState` values: `stopped`, `error`.
  ...
  - `app/routes/workers.py`: new `POST /api/v1/workers/{id}/stop` and `POST /api/v1/workers/{id}/restart` endpoints; archive/delete tear down the PVC for stateful workers.
  - Alembic migration: add `mode` column with default `ephemeral`, expand `worker_state` enum to include `stopped` and `error`.
- **Worker image**:
...
- **Frontend**:
  - `pages/Workers/`: new mode badge on worker cards; stop / restart buttons gated on state + mode.
  - `pages/Workers/CreateWorkerOverlay.tsx`: new mode selector (default `ephemeral`).
  - `pages/TaskBoard/`: matching stop/restart affordances on task-card worker controls.
  - `api/client.ts`: new `stopWorker(id)`, `restartWorker(id)` functions and a `mode` field on `Worker` types.
- **Infrastructure**:
