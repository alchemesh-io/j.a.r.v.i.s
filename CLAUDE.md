...
## Remote Sandbox Deployment (GKE)

Besides the local minikube cluster above, JARVIS also runs on a real GKE sandbox (`doctolib-app-tier2-dev`, cluster `t2-d-sbx-arch`), deployed from a **separate** repo: `doctolib/unified-healthcare-data-platform-infra`, config at `99_sbx_architecture/envs/doctolib-app-tier2-dev/` (Helm values at `argocd/jarvis/values.yaml`, CEL admission-policy whitelists at `files/cel-policies-values.yaml`).

- **ArgoCD owns the deployment, not Terraform.** The `jarvis` `Application` is generated by an `ApplicationSet` (app-of-apps pattern) that watches `argocd/*/argocd_config` paths in that repo. Terraform in that env only provisions supporting infra (namespace, IAM, Workload Identity bindings) via `jarvis.tf` — it never reads `argocd/jarvis/values.yaml` as file content, so changing image digests there has *no* Terraform-visible diff. `cel-policies-values.yaml` is different: it *is* read via `file()` into a real `helm_release.cel-policies` resource, so that specific file's changes do show up in `terraform plan`/`apply`.
- **Testing a PR branch before merge**: the ApplicationSet's watched branch is `var.argocd_watched_branch_name` (default `"main"`) in that env's `variables.tf`. Temporarily setting it to a PR branch name + `terraform apply` repoints ArgoCD's `targetRevision` at that branch (its `selfHeal: true` sync policy picks it up automatically, no separate `argocd app sync` needed) — precedented (a code comment references PR #6014 doing exactly this). **Must be reverted to `"main"` before merging** — the ApplicationSet's git generator fails to resolve a branch GitHub deletes on merge.
- **The 4 JARVIS images are whitelisted by exact path in *five* separate CEL policies** in `cel-policies-values.yaml`: `require-allowed-doctolib-repositories` (registry-origin — only needed for registries *not* already in that policy's built-in allowlist) plus `require-non-root-execution`, `require-drop-all-capabilities`, `require-readonly-root-filesystem`, `require-seccomp-runtime-default` (real container-hardening gaps, unrelated to registry origin). **All five need updating together** when an image's registry path changes — missing even one leaves a stale path that no longer matches the deployed image, and since these are Deny-mode admission policies, the pod gets blocked from scheduling entirely, not just a lint warning.
- Sandbox images currently live at `europe-west1-docker.pkg.dev/doctolib-artifacts/docker/*` (or `.../docker-branches/*` for non-`main` builds — a separate GCP Artifact Registry repo mirroring the same main-vs-staging AWS-account split ECR uses), replacing the old project-specific `europe-west4-docker.pkg.dev/d-platform-raytflf9/sbx-architecture/*` registry (DAD-33).
...
- Skills (`artifacts/skills/<name>/SKILL.md` + assets) are published as versioned `Skill`/`SkillRevision` resources via the Agent Registry Skills API — `make sync-artifacts-skills` zips each skill directory and calls `scripts/publish-skill.sh`, which creates the skill (draft, no revision) then attaches the payload via `gcloud alpha agent-registry skills revisions create --payload=...` and activates it. No GCS bucket involved — a prior revision of this repo published via `gcsSource` pointing at a dedicated bucket as a VPC-SC workaround; removed once the restricted-VIP DNS fix (below) confirmed the registry's own payload upload/download works directly
- Workers resolve skill metadata AND download the payload entirely through the `gcloud alpha agent-registry skills` command group (`list`/`describe`/`revisions describe`/`revisions download`) — no hand-rolled REST/curl. The registry's own `alt=media` download path 403s ("not available on Google's Restricted VIPs") when `agentregistry.googleapis.com` resolves to this VPC's *default* DNS answer, which lands in Google's **private** Private-Google-Access VIP pool (`199.36.153.4/30`) — a pool that does not support VPC Service Controls. Forcing resolution to the **restricted** VIP pool instead (`199.36.153.8/30`, the pool Google designates for VPC-SC-protected APIs) makes the exact same call succeed, confirmed live end-to-end (metadata + payload download + unzip). A pod-level `hostAliases` override is hard-denied by this cluster's `deny-host-aliases` ValidatingAdmissionPolicy, so the pin happens inside the container instead: `worker/entrypoint.sh` appends `199.36.153.8 agentregistry.googleapis.com` to `/etc/hosts` at startup (a per-pod file the kubelet mounts writable regardless of any container-level `readOnlyRootFilesystem`)
...
- Worker pods run **two containers** sharing a `/worker-state` emptyDir (`sizeLimit: 64Mi` — required by this cluster's `require-emptydir-sizelimit` policy):
  - `worker` — Claude Code running **interactively as the container's main process** (`tty: true`, `stdin: true`); the entrypoint provisions (config copy, repo clone, skill fetch from the Agent Registry) then `exec claude --dangerously-skip-permissions`. This is the Kubernetes Attach target for the browser terminal
...
- Worker pods are **never privileged** — skill fetches go through the Agent Registry API over authenticated HTTPS, so no docker daemon is needed even when skills are requested. The `status` sidecar is never privileged
- Terminal access:
...
| **This cluster denies pod-level `hostAliases`** | `deny-host-aliases` ValidatingAdmissionPolicy hard-rejects the `hostAliases` pod spec field outright | The one place this mattered (pinning `agentregistry.googleapis.com` to Google's restricted VIP) is done via an `/etc/hosts` append inside `worker/entrypoint.sh` instead — same effect, not a pod-spec field |
