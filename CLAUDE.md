...
| **Registry's own skill download path is VPC-SC-blocked** | `alt=media` against `agentregistry.googleapis.com` 403s for any in-cluster caller on this project. This is a VPC-SC access-level/perimeter identity check, NOT a DNS/routing issue — the hostname already resolves to Google's restricted VIP (`199.36.15x.x`) by default in this VPC, confirmed live, so pinning it further (e.g. a pod `hostAliases` entry) changes nothing. A `hostAliases` attempt was also hard-denied outright by this cluster's `deny-host-aliases` ValidatingAdmissionPolicy | Skills are published with a `gcsSource`; workers fetch the payload directly from GCS instead |
